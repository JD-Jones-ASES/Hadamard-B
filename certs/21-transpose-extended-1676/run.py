#!/usr/bin/env python3
"""cert 21 -- the three classes at order 1676 hold with the TRANSPOSE added
to the group.
  ...
  THEOREM (1676).  Let H be the decoded (s,i) = (1,1) bordered
  Goethals-Seidel record at order 1676 (certs 01/20), H' its Lemma-T
  i = 2 rebuild (certs 02/20), and H'' the orientation switch of H --
  the twelve off-diagonal core blocks negated, border unchanged
  (cert 20).  Then H, H' and H'' are pairwise inequivalent UNDER THE
  TRANSPOSE-EXTENDED RELATION.  So ORDER 1676 CARRIES AT LEAST THREE
  HADAMARD EQUIVALENCE CLASSES with the transpose in the group -- the
  statement cert 20 explicitly withheld.

  PROOF (finite, exact).  Every leg is an exact 4-profile comparison
  over all C(1676,4) = 327 588 749 775 row 4-subsets, in two
  arithmetics that agree bin for bin.  H ~~ H' is refuted by H !~ H'
  (68 bins, cert 20) and H !~ (H')^T (139 bins, new here); H ~~ H'' by
  H !~ H'' (70, cert 20) and H !~ (H'')^T (139, new); H' ~~ H'' by
  H' !~ H'' (66, cert 20) and H' !~ (H'')^T (139, new) -- and, the
  other route, (H')^T !~ H'' (139, new).  An invariant that differs is
  a separation.  []
  ...
  least separated pair by 66 bins.  FIVE classes are therefore
  EXHIBITED at 1676 by three constructions and transposition.  The
  ...
  1676 -- was NOT enumerated by the campaign and is NOT banked here.
  This certificate therefore says NOTHING about H vs H^T at 1676
...
WHERE THE PROFILES COME FROM, AND WHAT A DEFAULT RUN ESTABLISHES
  ...
  The two new C(1676,4) enumerations -- (H')^T and (H'')^T -- were not
  ...
  c2d-highcpu-16, prof42-1, us-east1-b, 2026-09-02) under the
  pre-registration experiments/pr0042/REGISTRATION.md, FLUSHED 10:17
  UTC before any matrix it governs was built, which fixed the objects
  (S2), the decision rules (S4: "differs in any bin => inequivalent";
  "equal in every bin => MEASURED, nothing proved") and the kill
  criteria (S5: blas != bits in any bin is a hard stop) in advance.
  The matrices were built AND verified at the desk
  (experiments/pr0042/build_matrices.py, every digest in its
  manifest.json, each through this repository's verify/verify.py); the
  rented machine enumerated and nothing else.  The DEFAULT path of this
  script AUDITS all ten banks.  Say "banked exact computation AUDITED"
  of a default run; the word "replayed" belongs to --full, which is
  OFFERED AND PRICED BELOW BUT HAS NOT BEEN RUN IN THIS REPOSITORY AT
  THIS ORDER (as in cert 20).
...
WHAT THIS SCRIPT DOES  (default path: standard library only, seconds)
  ...
  (2) Loads and AUDITS ten banked exact 4-profiles (five matrices x
      ...
      142 populated bins; total = C(1676,4); second moment =
      n^3(n-1)(n-2)/24, recomputed here AND compared against the field
      the bank declares; the schema, folding and impl the bank
      declares; and each bank's declared matrix digest against the
      in-process digest of the matrix rebuilt in THIS run.  Then
      blas == bits bin for bin on each of the five matrices.
  ...
  (5) Controls: five small Hadamard matrices profiled two ways, one of
      ...
      MEASURED, never asserted); the support control (the transposes
      at 1676 do NOT drop a bin as they do at 668 and 716 -- they SWAP
      two); the comparator in the null direction; a total-preserving
      corruption only the second moment can catch; and the dim-V trap
      on Sylvester H(16).
  (6) --full: recompute a transposed 1676 profile here from the rows
      clause [1] verified (certs/06-668-separation/full_recompute.py,
      imported by path, not copied, so certs 06/08/11/13/14/15/19/20/21
      cannot drift apart), after a smoke test against the forced
      profile of Sylvester H(128).  numpy is imported only under this
      flag, is finder-side only and is never in the trust chain; BLAS
      threads are capped at three before it loads.  PRICE, and why it
      has not been run: exactly cert 20's, since the order and the
      module are the same -- one 1676 leg is about 52x the 716 leg the
      same module took here (400 s), i.e. of order 6-7 h for one blas
      matrix at three threads, from the source laboratory's MEASURED
      sub-n^5 scaling (its 716->2060 ratio of 137, PR-0042
      REGISTRATION.md Amendment 1, exponent 4.66); on the Theta(n^5)
      law quoted elsewhere in this repository the same leg is 70x,
      about 7.8 h.  Both say hours; the smaller measured one is quoted
      so the price is not inflated in this certificate's own favour.
      And full_recompute.py materialises a C(n,2) x n pair matrix --
      at n = 1676 that is 2.35 GB as int8 and 9.4 GB as the float32
      copy the blas path makes, past this desk.  The bits path is the
      tractable one here.  NO --full LEG HAS BEEN RUN IN THIS
      REPOSITORY AT 1676.
...
# (cert 20's pin), and the two transposes are formed here from H' and H''.
# H^T is deliberately absent: the campaign never profiled it, so this
# certificate neither builds nor pins it.
SHA = {
    "dec":  "8e919c2bdb4d30c34817eb5650d2dd3d82d7c6504feccd96c5ca22a2191cdb99",
    "tw":   "6a4938371ddbe4ad8bd35f21d7e61dad683b15f8f2ec1c88e88ce579c4907405",
    "or":   "16d1617cc62532b26c010f3b174c741f0b9388089516759834030d9056a84346",
    "tw-T": "46f29432da47cb6e106a19d5c8e453e4bf797edb48924bfb0f9664a4af261ce3",
    "or-T": "ad870e1c216a76462c4913a667bb747847f64e2d197353d179f26a15ea85303d",
}
...
# this certificate's own (banked 2026-09-02 under cert 21); the six others
# are cert 20's, reused verbatim and re-pinned here.
# data/payload-records.json and data/twisted-i2-records.json are NOT
# file-pinned: they are shared with certs 01, 02 and 20, and the binding pin
# on each is the canonical digest of the matrix it produces, checked in
...
FILE_PINS = {
    ...
    "data/sep1676-twisted-T-exact-blas.json":
        "d951a78a0af94cb3979eec3878583d750ecb2c417946ab4c6159a3a7bfaaa040",
    "data/sep1676-twisted-T-exact-bits.json":
        "2a249d15e2d8c0aaa71d0fbeb503da43175aed7b9466c32dfaa77db1401bad6c",
    "data/sep1676-orient-T-exact-blas.json":
        "4d7506d93e35125d236432d92457d60321c1e1c4f014febc45e5ded0b06b820a",
    "data/sep1676-orient-T-exact-bits.json":
        "46750b71d6400a51ddfde19ec574cc6a8d92c345aabc521f5cd866ca3eb84e41",
}

PROFILES = {
    ("dec", "blas"):  "data/sep1676-decoded-exact-blas.json",
    ("dec", "bits"):  "data/sep1676-decoded-exact-bits.json",
    ("tw", "blas"):   "data/sep1676-twisted-exact-blas.json",
    ("tw", "bits"):   "data/sep1676-twisted-exact-bits.json",
    ("or", "blas"):   "data/sep1676-orient-exact-blas.json",
    ("or", "bits"):   "data/sep1676-orient-exact-bits.json",
    ("tw-T", "blas"): "data/sep1676-twisted-T-exact-blas.json",
    ("tw-T", "bits"): "data/sep1676-twisted-T-exact-bits.json",
    ("or-T", "blas"): "data/sep1676-orient-T-exact-blas.json",
    ("or-T", "bits"): "data/sep1676-orient-T-exact-bits.json",
}
...
# digest edited in.
MATRIX_NAME = {"dec": "H_1676-decoded", "tw": "H_1676-twisted",
               "or": "H_1676-orient", "tw-T": "H_1676-twisted-T",
               "or-T": "H_1676-orient-T"}

# Every profile at this order populates 142 bins -- but NOT the same 142.
# The three originals share one support and the two transposes share
# another; they differ in exactly two bins each way, so the union is 144.
# (Contrast 668 and 716, where each transpose DROPS one bin: cert 15, C3.)
NBINS = 142
NUNION = 144
ORIG_ONLY = [948, 964]        # populated in every original, in no transpose
TRANS_ONLY = [1180, 1204]     # populated in every transpose, in no original
...
# banked under cert 21; cert 20 = re-affirmed from that certificate's banks.
SEP = {
    ("dec", "tw"):    (68, 142, "cert 20"),
    ("dec", "or"):    (70, 142, "cert 20"),
    ("tw", "or"):     (66, 142, "cert 20"),
    ("dec", "tw-T"):  (139, 144, "NEW"),
    ("dec", "or-T"):  (139, 144, "NEW"),
    ("tw", "tw-T"):   (139, 144, "NEW"),
    ("tw", "or-T"):   (139, 144, "NEW"),
    ("or", "tw-T"):   (139, 144, "NEW"),
    ("or", "or-T"):   (139, 144, "NEW"),
    ("tw-T", "or-T"): (70, 142, "NEW"),
}
...
def verify_rows(tag, rows):
    ...
    os.remove(path)                    # 2.8 MB apiece; five of them
    return dig
...
def main(argv=None):
    ...
    ap.add_argument("--full", action="store_true",
                    help="RECOMPUTE a transposed exact 1676 profile here "
                         ...
                         "blas path wants ~9.4 GB; not yet run in this "
                         "repository")
    ...
    print("=" * 72)
    print("cert 21 -- order 1676: the three classes hold with the TRANSPOSE")
...
def _audit_bank(tag, impl, p, blob, built):
    ...
    check("%s  total == C(1676,4) == %d" % (lab, c_n_4(N)), tot == c_n_4(N))
    check("%s  second moment == n^3(n-1)(n-2)/24 == %d"
          % (lab, second_moment_want(N)), m2 == second_moment_want(N))
    check("%s  banked second_moment / total / n / C(n,4) headers agree with "
          "the recomputation" % lab,
    ...
    # T4 histogram is not an invariant -- NOTE-B.md S3.1) and which of the
    # two arithmetics produced it.
    check("%s  bank declares schema, the |T4| folding and its arithmetic"
          % lab,
          blob.get("schema") == "exact-4-profile/1"
          and str(blob.get("folded", "")).startswith("|T4|")
          and blob.get("impl") == impl,
          "%s / %s" % (blob.get("schema"), blob.get("impl")))
...
def _body(args, t_start):
    ...
    print("\n[1b] control C6 -- the dim-V trap on the real 1676 objects")
    dims = {t: dim_V_W(mats[t]) for t in sorted(mats)}
    check("C6  dim W (INVARIANT) is %d on all five 1676 objects, which "
    ...
    check("C6  dim V (NOT invariant) takes more than one value across them "
          ...
          ", ".join("%s %d" % (LABEL[t], dims[t][0]) for t in sorted(dims))
    ...
    for tag in ("dec", "tw", "or", "tw-T", "or-T"):
    ...
    for a, b in (("dec", "tw-T"), ("dec", "or-T"), ("tw", "or-T")):
        ...
        check("%-7s vs %-7s  the extreme tail does NOT separate them"
              % (LABEL[a], LABEL[b]),
              all(prof[(a, "blas")].get(k, 0) == prof[(b, "blas")].get(k, 0)
    ...
    check("order 1676: all %d pairs of the %d classes are separated under "
    ...
    print("    inequivalent.")
    tags5 = ["dec", "tw", "or", "tw-T", "or-T"]
    pairs5 = [(a, b) for i, a in enumerate(tags5) for b in tags5[i + 1:]]
    ...
    print("       populate two bins the originals do not, and lose two the")
    print("       originals have -- C3.)")

    print("\n  C3 -- the transposes are genuinely different objects, and at")
    print("        1676 they SWAP two bins rather than dropping one (668 and")
    print("        716 drop one: cert 15, C3)")
    orig = [set(prof[(t, "blas")]) for t in ("dec", "tw", "or")]
    trans = [set(prof[(t, "blas")]) for t in ("tw-T", "or-T")]
    check("C3  the three originals share one support of %d bins" % NBINS,
          orig[0] == orig[1] == orig[2] and len(orig[0]) == NBINS)
    check("C3  the two transposes share another support of %d bins" % NBINS,
          trans[0] == trans[1] and len(trans[0]) == NBINS)
    check("C3  the two supports differ: |T4| = %s populated in every "
          "original and no transpose" % ORIG_ONLY,
          sorted(orig[0] - trans[0]) == ORIG_ONLY,
          "counts %s" % [prof[("dec", "blas")][k] for k in ORIG_ONLY])
    ...
    check("C5  the corrupted profile still totals C(1676,4) -- so ONLY the "
    ...
    if args.full:
        ...
        print("\n[6] --full: RECOMPUTING transposed exact 1676 profiles here, "
              "from the rows")
        ...
        print("    float32 pair matrix, so NO --full leg has been run in "
              "this repository")
        print("    at 1676 -- cert 20's position, unchanged.  numpy is "
              "finder-side only")
        print("    and never in the trust chain.")

    # ---------------------------------------------------------- close out
    print("\n" + "=" * 72)
    print("checks run: %d" % NCHECK[0])
    if FAIL:
        print("cert 21: FAIL (%d): %s" % (len(FAIL), FAIL))
        return 1
    print("VERDICT: with the TRANSPOSE added to the group --")
    print("         ORDER 1676 CARRIES AT LEAST THREE HADAMARD EQUIVALENCE")
    print("         CLASSES (H, H', H''), the statement cert 20 explicitly")
    ...
    print("         C(1676,4) = %d row 4-subsets in two" % c_n_4(N))
    ...
    if replayed:
        ...
        print("         run, not recomputed.  The two new C(1676,4)")
    ...
    print("         anything about H vs H^T at 1676 (H^T was never")
    print("         profiled); anything at 1772 or 2060; any general theorem")
